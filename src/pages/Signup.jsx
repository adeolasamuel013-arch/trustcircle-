...
export default function Signup() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    setError('')
    if (!form.fullName || !form.email || !form.password || !form.skill) { setError('Please fill in all fields.'); return }
    if (form.password.length < 6) { setError('Password must be at least 6 characters.'); return }
    setLoading(true)
    try {
      await signUp(form.email, form.password, form.fullName, form.skill)
      setSuccess('Account created! Check your email for a confirmation link, then come back to log in.')
    } catch (err) {
  ...
  return (
    <div className="page-sm" style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
      <div style={{ width: '100%' }}>
        ...
        {success ? (
          <div style={{ background: 'var(--green-pale)', border: '1px solid var(--green-light)', borderRadius: 14, padding: '2rem', textAlign: 'center' }}>
            <p style={{ fontSize: 32, marginBottom: '1rem' }}>📧</p>
            <h3 style={{ color: 'var(--green)', marginBottom: '0.75rem', fontSize: 18 }}>Check your email!</h3>
            <p style={{ fontSize: 14, color: 'var(--muted)', lineHeight: 1.7, marginBottom: '1.5rem' }}>{success}</p>
            <Link to="/login"><button className="btn btn-green btn-full">Go to login</button></Link>
          </div>
        ) : (
