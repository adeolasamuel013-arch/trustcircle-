...
export default function Vouch() {
  ...
  async function handleVouch() {
    ...
    // Get FRESH score from DB (not cached)
    ...
    <div className="page-sm" style={{ padding: '2rem 1rem' }}>
      ...
      <div style={{ background: 'var(--green-pale)', border: '1px solid #B8E8D4', borderRadius: 12, padding: '1rem', marginBottom: '1.5rem' }}>
        <p style={{ fontSize: 13, color: 'var(--green-mid)', lineHeight: 1.6 }}>
          <strong>Your vouch weight: +{Math.max(5, Math.min(20, Math.floor((profile?.trust_score || 0) / 5) + 5))} pts</strong><br />
          This grows as your own trust score increases. You can only vouch for each person once.
        </p>
      </div>
      ...
      {results.length === 0 && search && !searching && (
        <p style={{ fontSize: 14, color: 'var(--muted)', textAlign: 'center', padding: '1rem 0' }}>
      ...
      {selected && (
        <div className="card" style={{ borderTop: '3px solid var(--green-light)', padding: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem', paddingBottom: '1rem', borderBottom: '1px solid var(--border)' }}>
            ...
            <button onClick={() => setSelected(null)} style={{ background: 'none', fontSize: 22, color: 'var(--muted)', border: 'none', cursor: 'pointer' }}>✕</button>
          </div>
          ...
          <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: '1rem' }}>
            Your vouch will add <strong>+{Math.max(5, Math.min(20, Math.floor((profile?.trust_score || 0) / 5) + 5))} points</strong> to their trust score.
          </p>
      ...
      {success && (
        <div style={{ background: 'var(--green-pale)', border: '1px solid #B8E8D4', borderRadius: 12, padding: '1rem 1.25rem', marginTop: '1rem' }}>
          <p style={{ fontSize: 14, color: 'var(--green-mid)', fontWeight: 500, lineHeight: 1.6 }}>✅ {success}</p>
        </div>
      )}
    </div>
  )
